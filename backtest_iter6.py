"""
backtest_iter6.py — Iteration 6.

Fixes vs iter-5:
 1. Volume bug: raw 5m volumes (~500k shares per bar for HDFCBANK) were being
    averaged against a per-day expectation. Correct metric: average 5m-bar
    volume > 20,000 shares (liquid enough for option scalping).
 2. Remove max trades/day cap (iter-4/5 capped at 2; market can have >2 legs).
 3. Keep: impulse (RVOL 2.0, range 1.2x ATR, close > 20-bar high, trend),
    pullback entry (higher low + hold SMA20 + re-entry confirmation),
    breakeven at +0.75 ATR, TP 1.5 ATR, SL impulse low, time stop 18 bars.
"""
import os
import sys
import numpy as np
...
P = {
    ...
    "cost_pct": 0.0010,
    "max_atr_pct": 0.035,
    "min_bar_vol": 20_000,     # avg 5m volume liquidity gate
}
...
def main():
    ...
    print("=== Iteration 6 results ===")
    for k, v in metrics(t).items():
        print(f"  {k}: {v}")
    t.to_csv("/home/ubuntu/nse_scalper/data/bt_iter6_trades.csv", index=False)
    print("\nBy exit reason:")
